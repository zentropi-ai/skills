...
- **Building Classifiers** — `labelers`, `versions`, `tests`, `check`,
  `benchmark`, `optimize`, `fork`, `star`. These are for creating and
  refining a labeler. They are meant **for development**,
  not high throughput.
- **Running Classifiers** — `label`. This is for classifying live content, including at
  production volume. Once you are satisfied with a labeler, **deploy a version** and use `label` for real traffic.

The typical lifecycle:

```
create labeler ──> add test samples ──> check / benchmark ──> optimize
       └──────────────> deploy a version ──────────────> label (production)
```
...
Because the CLI travels with the skill, the two never drift — re-downloading the
skill (see [Staying Up to Date](#staying-up-to-date)) updates both at once.
...
| `labelers list [--skip N] [--limit N]` | List your labelers. |
| `labelers get <id>` | Show one labeler. |
| `labelers create [--name] [--description] [--criteria TEXT] [--visibility public\|private]` | Create a labeler. `--criteria` sets the draft policy. |
| `labelers update <id> [--name] [--description] [--criteria] [--visibility]` | Update a labeler's draft. |
| `labelers delete <id>` | Delete a labeler. |
...
| `versions snapshot <labeler_id> [--version-name] [--description]` | Snapshot the **current draft** as a new version. The natural way to save a version. Use `deploy` to make it live. |
| `versions create <labeler_id> --criteria TEXT [--version-name] [--description]` | Snapshot **given criteria text** as a new version. Use `deploy` to make it live. |
...
| `benchmark <labeler_id> [--version draft\|latest\|<id>]` | Run the labeler over its stored test set; streams per-sample results then a summary. Defaults to your `draft`. |
| `optimize start <labeler_id> [--tier basic\|pro\|guru] [--version draft\|latest\|<id>]` | Start an AI-powered optimization job (default `--tier pro`, `--version draft`). Requires a subscription. See [Optimizer tiers](#optimizer-tiers). |
...
| `check <labeler_id> [--text TEXT] [--image SRC] [--video SRC] [--version draft\|latest\|<id>]` | Spot-check the labeler on ad-hoc content **without saving it**. `SRC` is a data URL, remote URL, or local file path (CLI encodes it). Defaults to your `draft` (or `latest` deployed if community labeler). |
| `label [labeler_id] [--criteria TEXT] [--text TEXT] [--image SRC] [--video SRC] [--version <labeler_version_id>\|latest] [--model cope-latest]` | Classify live content. Use this for higher volume. Pass a `labeler_id` (which defaults to `latest` deployed unless a `version_id` is specified). Alternatively, omit `labeler_id` and pass `--criteria` to label against ad-hoc policy text with no labeler. `SRC` is a data URL, remote URL, or local file path (CLI encodes it). Rate limits apply. |
...
deploy. `check` and `benchmark` can run a draft, but `label` only runs the
**deployed** version, so you must deploy before you can `label`.
...
to roll back, or to promote a snapshot you made with `versions snapshot` (or the
...
Once a version is deployed, classify live content. Use `label` (not
...
For a batch, call `label` once per item and aggregate. Retry on transient
...
labeler, call `label` but omit the `labeler_id` and pass `--criteria` with the policy
...
- **Using build commands for production.** `check` and `benchmark` are
  rate-limited dev tools. Deploy a version and use `label` for scaled traffic.
- **Forgetting to deploy.** `label --version latest` uses the *deployed*
  version. `versions snapshot` / `versions create` only snapshot criteria — run
...
# of this file — currently 0.10.0).
curl -s https://raw.githubusercontent.com/zentropi-ai/skills/main/zentropi-classifier/SKILL.md \
  | sed -n 's/^  version: "\(.*\)"/\1/p'
...
if git clone --depth 1 https://github.com/zentropi-ai/skills "$TMP" \
   && [ -f "$TMP/zentropi-classifier/SKILL.md" ]; then
  rm -rf "$DEST"/* && cp -r "$TMP/zentropi-classifier/." "$DEST"/
fi
rm -rf "$TMP"
```
...
- **This skill (latest)**: https://github.com/zentropi-ai/skills/tree/main/zentropi-classifier
